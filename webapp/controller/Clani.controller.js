sap.ui.define([
    ...
    "use strict";
    return Controller.extend("sap.ui.demo.wt.controller.Clani", {
        onInit : function () {
            ...
            sap.ui.core.UIComponent.getRouterFor(this).getRoute("Clani").attachPatternMatched(this.onRouteMatched, this);
        },

        onAfterRendering : function(){
            if(window.navigator.onLine){this.byId("bar").addStyleClass("bar")
            }
            else{
            ...
            if (!this.oDialog) {
				...
				this.oDialog = new sap.m.Dialog({
                    ...
                    content: new sap.m.Text({
                        text:"Ali želite odstraniti člana?"
                    }),
                    endButton: new sap.m.Button({
                        text: "Da",
                        press: function() {
                            that.oDialog.close();
                            removeUser(that.byId("claniField").getValue(), that);
                        }
                    }),
		...
		onSuggest: function (event) {
			...
			this.byId("claniField").getBinding("suggestionItems").filter(aFilters);
			this.byId("claniField").suggest();
		},

        addMember: function(){
            var user = checkAddMemberData(this);

            if(user.valid){
                userExists(user.ime, user.priimek, user.username, user.password, this);
            }
        }
    });
 });